...
class TimerThread
{
   ...
   public:
    TimerThread() {}
    TimerThread(TimerCallbackFunction_t callBack, const char* const pcName, TickType_t period, bool autoReload = true,
                void* const id = 0)
        : _callBack(callBack), _pcName(pcName), _period(period), _timer(0x00)
    {
        if (debug)
        {
    ...
    TickType_t Period() { return _period; }
